...
/**
 * Loaded on demand (next/dynamic) and mounted only while open.
...
export default function ContactModal({ onClose }: ContactModalProps) {
  ...
  const handleCopy = async (text: string, id: string) => {
    try {
      ...
      // Clipboard can be unavailable (insecure context / permissions); still confirm.
    }
    setCopiedItem(id);
    window.clearTimeout(copyTimer.current);
    copyTimer.current = window.setTimeout(() => setCopiedItem(null), 2000);
  };
  ...
  return (
    <div
      ...
      className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 md:p-6 lg:p-10"
    >
      {/* Backdrop. A full-screen backdrop-filter blur re-sampled the whole page
          (including the WebGL hero) every frame the modal content scrolled; at
          92% black the blur was barely visible, so it is now a plain scrim. */}
      <div
        onClick={onClose}
        className="modal-backdrop-in fixed inset-0 bg-black/[0.92] cursor-pointer"
        aria-hidden="true"
      />

      {/* Modal Dialog Card */}
      <div ref={panelRef} className="modal-panel-in relative w-full max-w-3xl max-h-[92dvh] sm:max-h-[88dvh] overflow-y-auto overscroll-contain [-webkit-overflow-scrolling:touch] rounded-t-3xl sm:rounded-3xl bg-[#0b0d13] border border-white/10 shadow-[0_0_80px_rgba(0,0,0,0.95)] p-5 sm:p-8 lg:p-10 pb-8 sm:pb-8 lg:pb-10 pb-safe z-10 text-white scrollbar-thin scrollbar-thumb-white/10">
